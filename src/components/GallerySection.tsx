...
function PhotoCard({ photoUrl, idx, style, photoDate }: PhotoCardProps) {
  ...
  const messages = [
    "รูปหวานเซ็ทแรก ><",
    "แถมไก่เน่ามาให้ คิๆ",
    "ชวนไปตีแบตบ่อยๆ รับรักเราได้ละม้าง (อ้อ รับแล้ว)",
    "มองจากดาวอังคาร ก็คิดว่าคบกันแล้ว",
    "แต่ตอนนี้มีแฟนแล้วอะดิ",
    "อ้าว 10 เดือนแล้วหรอ",
  ];

  // ฟังก์ชันช่วยจัดการตำแหน่งโฟกัสของรูปภาพแต่ละใบ
  const getImagePositionClass = (index: number) => {
    if (index === 2) return 'object-bottom'; 
    ...
    >
      ...
      >
        ...
        <div className="absolute inset-3 bottom-12" style={{ backfaceVisibility: "hidden" }}>
          {idx < 2 && (
            <div className="absolute -top-4 left-1/2 -translate-x-1/2 w-16 h-8 bg-[#f4a7be]/20 backdrop-blur-[2px] rotate-[15deg] border-x border-[#f4a7be]/10 z-20" />
          )}

          <div className="relative w-full h-full bg-[#fce4ec] overflow-hidden rounded-sm pointer-events-none">
            <img 
              src={photoUrl} 
              className={`w-full h-full object-cover ${getImagePositionClass(idx)}`} 
              alt="Memory" 
            />
          </div>
        ...
        <div
          className="absolute inset-1 bottom-12 flex flex-col items-center justify-center p-1 bg-[#fff8fb] rounded-sm overflow-hidden"
          style={{ backfaceVisibility: "hidden", transform: "rotateY(180deg)" }}
        >
          <span className="text-[#e8789a] mb-1 text-base flex-shrink-0">♥</span>
          <div className="flex-1 w-full flex items-center justify-center overflow-y-auto px-1.5 custom-scrollbar" style={{ scrollbarWidth: 'none' }}>
            <p
              className="text-[#c2547a] font-bold text-center italic w-full break-words whitespace-pre-line text-pretty px-0.5"
              style={{

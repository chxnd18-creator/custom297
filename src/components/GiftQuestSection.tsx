...
export default function GiftQuestSection({ onBack, onFinish }: { onBack: () => void, onFinish: () => void }) {
    ...
    const GIFTS = [
        "เลี้ยงข้าวฟรี 2 วัน 💋", "นวดสปาสักที่ 💐", "หอมแก้มไม่จำกัด 🫶", "อาทิตย์นี้ไปนอนด้วยที่บ้าน 😘",
        "ไปรับกลับบ้านทั้งอาทิตย์ 💝", "นีน่าไม่กินหวาน 1 อาทิตย์ 🎁", "ถ้าทะเลาะกันง้อทันที 2 ครั้ง ✨", "ตามใจ 1 วัน 😻"
    ];
                            ...
                            <div key={card.id} onClick={() => handleCardClick(card.id)} className="aspect-square cursor-pointer relative z-50" style={{ perspective: '1000px' }}>
                                <motion.div animate={{ rotateY: card.flipped || card.matched ? 180 : 0 }} transition={{ duration: 0.4 }} className="w-full h-full relative" style={{ transformStyle: 'preserve-3d' }}>
                                    ...
                                    <div className="absolute inset-0 bg-[#fff8fb] rounded-xl overflow-hidden shadow-2xl" style={{ backfaceVisibility: 'hidden', WebkitBackfaceVisibility: 'hidden', transform: 'rotateY(180deg)' }}>
                                        {/* 🎯 ไฮไลต์จุดแก้ไข: เช็กถ้าเป็นรูป 7.jpeg ให้เน้นส่วนล่างด้วย object-bottom */}
                                        <img 
                                            src={card.img} 
                                            alt="couple" 
                                            className={`w-full h-full object-cover ${card.img === "/images/7.jpeg" ? "object-bottom" : "object-center"}`} 
                                        />
                                    </div>
                                </motion.div>
                            </div>
                ...
                {result && (
                    ...
                    >
                        <motion.div 
                            ...
                            className="bg-[#fff8fb] p-8 md:p-12 rounded-[40px] text-center w-full max-w-[420px] shadow-[0_20px_80px_rgba(232,120,154,0.3)] border-8 border-[#f4a7be] flex flex-col items-center justify-center min-h-[380px]"
                        >
                            <motion.span 
                                animate={{ y: [0, -15, 0] }}
